...
def groq_chat(
    messages: list[dict], model: str = GROQ_MODEL, temperature: float = 0.3, max_tokens: int = 2048
) -> Optional[str]:
...
def send_email(to: str, subject: str, html: str, from_addr: str = RESEND_FROM) -> bool:
...
def supabase_insert(table: str, row: dict) -> Optional[dict]:
    ...
    try:
        ...
        r = httpx.post(
            f"{SUPABASE_URL}/rest/v1/{table}", headers=_sb_headers(), json=row, timeout=30
        )
        r.raise_for_status()
        data = r.json()
...
def deploy_worker(worker_name: str, script: str, routes: Optional[list[str]] = None) -> bool:
    """
